...
USER root
RUN apt-get update && apt-get -y install --no-install-recommends wget apt-utils docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin && rm -rf /var/lib/apt/lists/*
